...
public class NormalVehicleServiceScreen extends AppCompatActivity {

    private EditText make_details_input, model_details_input, year_details_input, vin_num_input, date_input;
    private Button request_quote_btn;

    private String URL = "http://10.0.2.2/dauvanDB/vehicle_services.php";
    private String make_details, model_details, year_details, vin_num, date;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.vehicle_normal_service_screen);
        ...
        make_details = model_details = year_details = vin_num = date = "";
        ...
        backButton.setOnClickListener(view -> {
            Intent intent = new Intent(NormalVehicleServiceScreen.this, MainVehicleScreen.class);
            startActivity(intent);
        });
    }

        // vehicle_service method to handle user vehicle input with MySQL and Volley
        public void vehicle_service(View view) {
            make_details = make_details_input.getText().toString().trim();
            model_details = model_details_input.getText().toString().trim();
            year_details = year_details_input.getText().toString().trim();
            vin_num = vin_num_input.getText().toString().trim();
            ...
            if (!model_details.equals("") && !make_details.equals("") && !year_details.equals("") && !vin_num.equals("") && !date.equals("")) {
                StringRequest stringRequest = new StringRequest(Request.Method.POST, URL, new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        if (response.equals("success")) {
                            // Show success message
                            Toast.makeText(NormalVehicleServiceScreen.this, "Request quote successful!", Toast.LENGTH_SHORT).show();
                            ...
                            Toast.makeText(NormalVehicleServiceScreen.this, "Failed to request quote. Please try again.", Toast.LENGTH_SHORT).show();
                        }
                    }
                    ...
                    public void onErrorResponse(VolleyError error) {
                        // Handle Volley error
                        Toast.makeText(NormalVehicleServiceScreen.this, "Normal Vehicle Service Failed", Toast.LENGTH_SHORT).show();
                    }
                }) {
                    @Override
